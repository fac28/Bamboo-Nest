...
export async function ReviewSeller({seller_id}: {seller_id: string}) {
  ...
  const {data: sellerData} = await supabase.from('users').select('first_name').eq('id', seller_id)
  let seller_name= ''
  ...
  const submit = async (formData: FormData) => {
    ...
    const review_score = Math.round(Number(formData.get('review-score')) / 25 + 1)
  ...
  return (
    <div>
      ...
      <form className="grid grid-cols-1 gap-2">
        <input id='seller-id' name='seller-id' value={seller_id} className='hidden'/>
        <label htmlFor='review'>Review</label>
        <input type="range" id='review-score' name='review-score' list='values'/>
